...
class Windows10SystemLogger:
    ...
    def session_suspension_time_set_errmsg(self, success):
        if not success:
           ...
           self.log.write("The amount of idle time required before suspending a session must be configured to 15 minutes or less.\n\n")
    ...
    def apci_data_collection_disabled_errmsg(self, success):
        if not success:
           ...
           self.log.write("The Application Compatibility Program Inventory must be prevented from collecting data and sending the information to Microsoft.\n\n")
    ...
    def netbios_name_ignored_errmsg(self, success):
        if not success:
           ...
           self.log.write("The system must be configured to ignore NetBIOS name release requests except from WINS servers.\n\n")

    def toast_notification_disabled_errmsg(self, success):
        if not success:
           self.log.write("Check SV-78329r1_rule: ")
           self.log.write("Toast notifications to the lock screen must be turned off.\n\n")

    def global_system_objets_permissions_disabled_errmsg(self, success):
        if not success:
           self.log.write("Check SV-80171r1_rule: ")
           self.log.write("Windows Update must not obtain updates from other PCs on the Internet.\n\n")