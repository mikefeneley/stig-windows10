...
from windows10_tmp_sys_logger import Windows10TmpSysLogger
from winreg_comparator import WinRegComparator


class Windows10TmpSysAuditor:
    ...
    def emet_deephooks_set(self):
        """
        Check SV-77907r2_rule: The Enhanced Mitigation Experience Toolkit
        (EMET) Default Actions and Mitigations Settings must enable Deep
        Hooks.
    ...
    def hardware_device_pfw_enabled(self):
        """
        Check SV-78207r2_rule: The use of a hardware security device with
        Microsoft Passport for Work must be enabled.
    ...
    def client_rpc_authentication_set(self):
        ...
        key_val = "EnableAuthEpResolution"
        val = 1
        compare = self.comparator.reg_equals(None, key, subkey, key_val, val)
        return compare
    ...
    def error_reports_generated(self):
        """
        Check SV-77949r1_rule: The system must be configured to generate error
        reports.
    ...
    def smb_packet_signing(self):
        """
        Check SV-78197r1_rule: The Windows SMB client must be enabled to perform
    ...
    def inprivate_browsing_disabled(self):
        """
        Check SV-78195r1_rule: InPrivate browsing in Microsoft Edge must be
        disabled.
    ...
    def num_error_reports_configured(self):
        ...
        val = 100
        compare = self.comparator.reg_greater_equal_than(
            None, key, subkey, key_val, val)
        return compare
    ...
    def queue_error_reports_disabled(self):
        """
        Check SV-78037r1_rule: The display of slide shows on the lock screen
        must be disabled.
    ...
    def windows_telemetry_data_set(self):
        """
        Check SV-78173r1_rule: Windows Telemetry must be configured to the
        lowest level of data sent to Microsoft.
    ...
    def pku2u_authentication_disabled(self):
        """
        Check SV-78257r1_rule: PKU2U authentication using online identities must
        be prevented.
    ...
    def consent_override_behavior_set(self):
        """
        Check SV-78065r1_rule: The system must be configured to permit the
        default consent levels of Windows Error Reporting to override any other
        consent policy setting.
    ...
    def pin_length_configuered(self):
        """
        Check SV-78211r1_rule: The minimum pin length for Microsoft Passport for
         Work must be 6 characters or greater.
    ...
    def password_storage_disabled(self):
        """
        Check SV-78243r1_rule: The system must be configured to prevent the
    ...
    def elevated_network_domain_privlidge_disabled(self):
        """
        Check SV-78087r1_rule: Local administrator accounts must have their
        privileged token filtered to prevent elevated privileges from being used
        over the network on domain systems.
    ...
    def http_printer_driver_dl_disabled(self):
        """
        Check SV-78105r1_rule: Downloading print driver packages over HTTP must
        be prevented.
    ...
    def blank_passwd_accounts_disabled(self):
        """
        Check SV-78107r1_rule: Local accounts with blank passwords must be
        restricted to prevent access from the network.
    ...
    def emet_antidetours_set(self):
        """
        Check SV-77915r2_rule: The Enhanced Mitigation Experience Toolkit (EMET)
        Default Actions and Mitigations Settings must enable Anti Detours.
    ...
    def uac_admin_mode_enabled(self):
        """
        Check SV-78319r1_rule: User Account Control must run all administrators
    ...
    def sys_event_log_size_configuered(self):
        """
        Check SV-78017r1_rule: The System event log size must be configured to
        32768 KB or greater.
    ...
    def uac_elevate_restricted(self):
        """
        Check SV-78317r1_rule: User Account Control must only elevate UIAccess
    ...
    def uac_installer_detection_enabled(self):
        """
        Check SV-78315r1_rule: User Account Control must be configured to detect
    ...
    def kerberos_encrypt_configuered(self):
        """
        Check SV-78285r1_rule: Kerberos encryption types must be configured to
        prevent the use of DES and RC4 encryption suites.
    ...
    def smb_packet_signing_required(self):
        """
        Check SV-78213r1_rule: The Windows SMB server must perform SMB packet
    ...
    def error_report_ssl_required(self):
        """
        Check SV-78015r1_rule: The system must be configured to use SSL to
        forward error reports.
    ...
    def domain_joined_computers_unenumerated(self):
        """
        Check SV-78121r1_rule: Connected users on domain-joined computers must
        not be enumerated.
    ...
    def max_error_queue_reports_set(self):
        """
        Check SV-78051r1_rule: The maximum number of error reports to queue on a
        system must be configured to 50 or greater.
    ...
    def security_event_log_size_configuered(self):
        """
        Check SV-78013r1_rule: The Security event log size must be configured to
    ...
    def rss_feed_attachements_disabled(self):
        """
        Check SV-78233r1_rule: Attachments must be prevented from being
        downloaded from RSS feeds.
    ...
    def admin_account_elevation_enumeration_disabled(self):
        """
        Check SV-78169r1_rule: Administrator accounts must not be enumerated
        during elevation
    ...
    def user_errmsg_disabled(self):
        """
        Check SV-77995r1_rule: The system must be configured to prevent the
    ...
    def ignore_edge_warnings_disabled(self):
        """
        Check SV-78189r1_rule: Users must not be allowed to ignore SmartScreen
    ...
    def wizard_provider_dl_disabled(self):
        """
        Check SV-78111r1_rule: Web publishing and online ordering wizards must
        be prevented from downloading a list of providers.
    ...
    def nondomain_domain_network_blocked(self):
        """
        Check SV-78075r1_rule: Connections to non-domain networks when connected
        to a domain authenticated network must be blocked.
    ...
    def nui_disabled(self):
        """
        Check SV-78119r1_rule: The network selection user interface (UI) must
        not be displayed on the logon screen.
    ...
    def rds_encryption_level_set(self):
        """
        Check SV-78231r1_rule: Remote Desktop Services must be configured with
        the client connection encryption set to the required level.
    ...
    def uac_virtalilzation_set(self):
        """
        Check SV-78321r1_rule: User Account Control must virtualize file and
    ...
    def daily_error_reports_required(self):
        """
        Check SV-78055r1_rule: The system must be configured to attempt to
        forward queued error reports once a day.
    ...
    def annonymous_users_excluded(self):
        """
        Check SV-78245r1_rule: The system must be configured to prevent
        anonymous users from having the same rights as the Everyone group.
    ...
    def error_report_archive_configuered(self):
        """
        Check SV-78029r1_rule: The system must be configured to store all data
        in the error report archive.
    ...
    def uac_elevation_requests_disabled(self):
        """
        Check SV-78311r1_rule: User Account Control must automatically deny
        elevation requests for standard users.
    ...
    def smb_insecure_login_disabled(self):
        """
        Check SV-78059r1_rule: Insecure logons to an SMB server must be
        disabled.
    ...
    def error_reports_archived(self):
        """
        Check SV-78025r1_rule: The system must be configured to archive error
        reports.
    ...
    def remote_desktop_host_secure_rpc_required(self):
        """
        Check SV-78227r1_rule: The Remote Desktop Session Host must require
    ...
    def spn_client_accept_configuered(self):
        """
        Check SV-78225r1_rule: The service principal name (SPN) target name
        validation level must be configured to Accept if provided by client.
    ...
    def rsd_passwd_prompt_required(self):
        """
        Check SV-78223r1_rule: Remote Desktop Services must always prompt a
        client for passwords upon connection.
    ...
    def remote_desktop_session_hosts_local_drive_disabled(self):
        """
        Check SV-78221r1_rule: Local drives must be prevented from sharing with
    ...
    def outgoing_traffic_secured(self):
        """
        Check SV-78129r1_rule: Outgoing secure channel traffic must be encrypted
        or signed.
    ...
    def local_user_enumeration_disabled(self):
        """
        Check SV-78123r1_rule: Local users on domain-joined computers must not
        be enumerated.
    ...
    def emet_banned_functions_disabled(self):
        """
        Check SV-77923r2_rule: The Enhanced Mitigation Experience Toolkit (EMET)
        Default Actions and Mitigations Settings must enable Banned Functions.
    ...
    def ldap_client_signing_level_set(self):
        """
        Check SV-78293r1_rule: The system must be configured to the required
        LDAP client signing level.
    ...
    def ntlm_ssp_client_session_security_configuered(self):
        """
        Check SV-78295r1_rule: The system must be configured to meet the minimum
        session security requirement for NTLM SSP based clients.
    ...
    def ntlm_ssp_server_session_security_configuered(self):
        """
        Check SV-78297r1_rule: The system must be configured to meet the minimum
        session security requirement for NTLM SSP based servers.
    ...
    def uac_approval_mode_enabled(self):
        """
        Check SV-78307r1_rule: User Account Control approval mode for the
        built-in Administrator must be compare.
    ...
    def ac_sleep_wakeup_password_required(self):
        """
        Check SV-78139r1_rule: The user must be prompted for a password on
        resume from sleep (plugged in).
    ...
    def case_insensitivity_required(self):
        """
        Check SV-78303r1_rule: The system must be configured to require case
    ...
    def fips_compliant_algorithims_set(self):
        """
        Check SV-78301r1_rule: The system must be configured to use
    ...
    def outgoing_secure_traffic_encrypted(self):
        """
        Check SV-78133r1_rule: Outgoing secure channel traffic must be
    ...
    def untrusted_fonts_blocked(self):
        """
        Check SV-78131r1_rule: The system must be configured to block untrusted
        fonts from loading.
    ...
    def outgoing_traffic_signed(self):
        """
        Check SV-78137r1_rule: Outgoing secure channel traffic must be signed
        when possible.
    ...
    def remote_desktop_client_password_unsaved(self):
        """
        Check SV-78219r1_rule: Passwords must not be saved in the Remote
        Desktop Client.
    ...
    def machine_lockout_enabled(self):
        """
        Check SV-78447r1_rule: The machine account lockout threshold must be set
        to 10 on systems with BitLocker enabled.
        ...
        NOTE:
            If Bitlocker is not enabled, this is NA. Add check later.
        """
        key = HKEY_LOCAL_MACHINE
        subkey = r"SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System"
    ...
    def session_suspension_time_set(self):
        """
        Check SV-78205r1_rule: The amount of idle time required before
        suspending a session must be configured to 15 minutes or less.
        ...
        val = 15
        compare = self.comparator.reg_less_equal_than(
            None, key, subkey, key_val, val)
        return compare

    def password_reset_enabled(self):
        """
        Check SV-78143r1_rule: The computer account password must not be
    ...
    def password_age_configured(self):
        """
        Check SV-78151r1_rule: The maximum age for machine account passwords
        must be configured to 30 days or less.
        ...
        compare = self.comparator.reg_less_equal_than(
            None, key, subkey, key_val, val)
        equals_zero = self.comparator.reg_equals(None, key, subkey, key_val, 0)
        return compare and not equals_zero
    ...
    def global_system_objets_permissions_disabeled(self):
        """
        Check SV-80171r1_rule: Windows Update must not obtain updates from other
        PCs on the Internet.
...
if __name__ == "__main__":
    auditor = Windows10TmpSysAuditor()
    auditor.audit()